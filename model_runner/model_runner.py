"""
RoboPacerV2 - Model Runner (steering-only debug/test tool)
==============================================================
...
This is the standalone steering-only tool (formerly main/main.py) - for
normal driving with distance/pace targets, see main/main.py, which combines
this same model-steering logic with cruise_control.py's speed control.

Usage:
    python3 model_runner.py                  headless (no window) - fastest,
                                      prints average FPS once per second to
                                      the console
    python3 model_runner.py --display        opens a live cv2 preview window
                                      (costs a few ms/frame - see
                                      DISPLAY_EVERY_N_FRAMES)
    python3 model_runner.py --raw-steering   sends the model's raw per-frame
                                      prediction straight to the servo - no
                                      EMA smoothing, no deadzone. For
                                      testing/debugging what the model
                                      itself outputs, not for normal driving.
...
fight it and the motor will never actually respond. Always launch with:
    ...
    python3 /home/pi/RoboPacerV2/model_runner/model_runner.py
--------------------------------------------------------------------------
"""
...
LOG_FILE_PATH = os.path.join(BASE_DIR, "model_runner.log")
