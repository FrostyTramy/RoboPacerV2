...
from config.ipc_config import ODO_SOCKET, RELAY_SOCKET, stop_grace_seconds
...
# Scripturi excluse de la kill-ul de ESTOP desi ruleaza sub REPO_ROOT:
...
def _find_target_processes():
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        ...
        try:
            ...
            if not is_python or not any(REPO_ROOT in arg for arg in cmdline):
                continue
...
def kill_target_processes():
    ...
    # Relay is already OFF (that's what ESTOP means), so giving a script time
    # to finish its own cleanup is safe - data_recorder needs it to save.
    wait_seconds = 0
    for proc in procs:
        try:
            args = proc.cmdline()
            ...
            args = []
        cmdline = " ".join(args) or "?"
        wait_seconds = max(wait_seconds, stop_grace_seconds(args))
    ...
    gone, alive = psutil.wait_procs(procs, timeout=wait_seconds)
    for proc in gone:
